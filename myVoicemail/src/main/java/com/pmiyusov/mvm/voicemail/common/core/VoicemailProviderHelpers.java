...
public final class VoicemailProviderHelpers implements VoicemailProviderHelper {
    ...
    public Uri insert(Voicemail voicemail) {
        ...
        Log.d(TAG, String.format("Inserting new voicemail: %s", voicemail));
    ...
    public int update(Uri uri, Voicemail voicemail) {
        ...
        Log.d(TAG, "Updating voicemail: " + voicemail + " for uri: " + uri);
    ...
    private void setVoicemailContent(Uri voicemailUri, byte[] inputBytes, InputStream inputStream,
        ...
        Log.d(TAG, String.format("Writing new voicemail content: %s", voicemailUri));
    ...
    public Voicemail findVoicemailBySourceData(String sourceData) {
        ...
        try {
            ...
            if (cursor.getCount() != 1) {
                Log.w(TAG, "Expected 1 voicemail matching sourceData " + sourceData + ", got " +
                        cursor.getCount());
                return null;
            }
            cursor.moveToFirst();
    ...
    public Voicemail findVoicemailByUri(Uri uri) {
        ...
        try {
            ...
            if (cursor.getCount() != 1) {
                Log.w(TAG, "Expected 1 voicemail matching uri " + uri + ", got " + cursor.getCount());
                return null;
            }
            cursor.moveToFirst();
            ...
            if (voicemail.getUri().equals(uri)) {
                ...
                Log.w(TAG, "Queried uri: " + uri + " do not represent a unique voicemail record.");
                return null;
            }
        } finally {
            CloseUtils.closeQuietly(cursor);
    ...
    public int deleteAll() {
        Log.i(TAG, String.format("Deleting all voicemails"));
